...
#   CLAWNIX_URL                 — ClawNix WebUI base URL (default: http://localhost:3344)
#   CLAWNIX_APPROVAL_TIMEOUT    — Max seconds to wait (default: 300)
...
CLAWNIX_URL="${CLAWNIX_URL:-http://localhost:3344}"
TIMEOUT="${CLAWNIX_APPROVAL_TIMEOUT:-300}"
TOOL="${CLAUDE_TOOL:-unknown}"
INPUT="${CLAUDE_INPUT:-}"
SESSION="${CLAUDE_SESSION:-unknown}"

# Request approval
RESPONSE=$(curl -sf -X POST "$CLAWNIX_URL/api/approve" \
...
for i in $(seq 1 "$TIMEOUT"); do
  STATUS=$(curl -sf "$CLAWNIX_URL/api/approve/$REQUEST_ID" 2>/dev/null || echo '{}')
